...
def add_to_cart(request):
    if request.method == 'POST':
        ...
        if cart:
            basket= Cart.objects.filter( user__username=request.user.username, paid=False, product=product .id,
            quantity =quantity, price=new_price ).first()
            if basket:
...
def payment(request):
    if request.method == 'POST':
        ...
        #Collect data to send to pay stack via call url
